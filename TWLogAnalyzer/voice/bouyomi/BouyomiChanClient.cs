...
namespace FNF.Utility
{
    ...
    public class BouyomiChanClient : IDisposable
    {
        ...
        {
            ClientChannel = new IpcClientChannel("BouyomiChan", null); //チャンネル名は何でもいい
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (ClientChannel != null)
            {
                ...
                {
                    // GCでUnregisterChannelで未登録だからダメよのエラーが出ることがある
                    // 上からの呼び出し方が悪いと思うが、原因特定ができていないため一時対策としてエラーを握りつぶす
                    TWLogAnalyzer.common.Logger.Instance.PutMessage($"棒読みちゃんのDisposeエラー: {ex.Message}");
                }
            }
        }
